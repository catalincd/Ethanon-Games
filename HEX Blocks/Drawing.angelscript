...
void DrawRectangleSpriteStroke(vector2 p1, vector2 p2, float size, uint color)
{
	...
	DrawShapedSprite(dataBase.pixel, p1-offset, vector2(SizeX, size), color); //up
	DrawShapedSprite(dataBase.pixel, vector2(p1.x, p2.y)-offset, vector2(SizeX, size), color); //down
	DrawShapedSprite(dataBase.pixel, p1-offset, vector2(size, SizeY), color); //left
	DrawShapedSprite(dataBase.pixel, vector2(p2.x, p1.y)-offset, vector2(size, SizeY+size), color); //right	
}
