class DataBase
{
	...
	void loadTime()
	{
		const string path = GetExternalStorageDirectory()+"time.kta";
		
		if(FileExists(path))
		{
			const string str = GetStringFromFile(path);
			time.parseString(str);
			string toSet = time.get("DATA", "TIME");
			g_timeManager.set(toSet);
		}
		g_timeManager.timeLoaded = true;
	}
	
	void saveTime()
	{
		if(g_timeManager.timeLoaded)
		{
			enmlEntity timeEn;
			timeEn.add("TIME", g_timeManager.get());
			time.addEntity("DATA", timeEn);
			string str = time.generateString();
			SaveStringToFile(GetExternalStorageDirectory()+"time.kta", str);
		}
	}
