...
class StatsScene : Scene
{
	...
	void onCreated()
	{
		...
		strokePos = vector2(SCREEN_SIZE_X2, 250*GetScale());
		...
		textPos2 = vector2(SCREEN_SIZE_X2, SCREEN_SIZE_Y+35*GetScale()) - (ComputeTextBoxSize(dataBase.font40, "BACK TO MAIN MENU")/2*GetScale());
		...
		text_total_pos = vector2(lPos, 350*GetScale())+vector2(text_total_size.x,0);
		text_current_pos = vector2(lPos, 425*GetScale())+vector2(text_current_size.x,0);
		text_best_pos = vector2(lPos, 500*GetScale())+vector2(text_best_size.x,0);
		@m_start = UseButton(vector2(SCREEN_SIZE_X2, SCREEN_SIZE_Y*0.75));
		m_start.setText(" START ", dataBase.font40);
	...
	void onUpdate()
	{
		...
		//DrawText(textPos+index, , dataBase.font40, whitee, GetScale());
		
		DrawShapedSprite(g_gameManager.blockS, sprPos2-index, navSize, realWhite);
		DrawText(textPos2-index, "BACK TO MAIN MENU", dataBase.font40, whitee, GetScale());
		...
		m_start.setColor(ARGB(alpha, SEC));
		m_start.putButton();
